...
df_5["CPI Division"] = round(df_5["Old_Division_CPI_26_06_2026"] * df_5["Weight_Price_Division_today"] / df_5["Weight_Price_Division_reference"], 4) #chain-link the old with the new/revised Division CPI series
...
if current_day_obj.weekday() == 0:
    # Code to execute on Monday
    previous_day_obj = current_day_obj - timedelta(days = 3)
else:
...
print(len(unique_divisions))

for unique_ in unique_divisions:
    
    df_13 = prior_df.loc[prior_df["Division"] == unique_]["CPI Division"].iloc[0]
    df_14 = current_df.loc[current_df["Division"] == unique_]["CPI Division"].iloc[0]
...
unique_subclasses = current_df['Subclass'].unique()
print(len(unique_subclasses))

for unique_ in unique_subclasses:
    
    df_15 = prior_df.loc[prior_df["Subclass"] == unique_]["CPI Division"].iloc[0]
    df_16 = current_df.loc[current_df["Subclass"] == unique_]["CPI Division"].iloc[0]
...
if is_last_thursday(current_date_str):
    ...
    for unique_ in unique_divisions:
        ...
        df_17 = prior_df.loc[prior_df["Division"] == unique_]["CPI Division"].iloc[0]
        df_18 = current_df.loc[current_df["Division"] == unique_]["CPI Division"].iloc[0]
...
while today_p <= end_date:
    ...
    # Skip weekends
    if today_p.weekday() < 5:      # 0=Mon, ..., 4=Fri
        ...
        df_5 = pd.merge(index_, df_4, on = 'Division')
        #df_5["CPI Division"] = round(100 * df_5["Weight_Price_Division_today"] / df_5["Weight_Price_Division_reference"], 4)
        df_5["CPI Division"] = round(df_5["Old_Division_CPI_26_06_2026"] * df_5["Weight_Price_Division_today"] / df_5["Weight_Price_Division_reference"], 4) #chain-link the old and new/revised Division CPI series
        df_5 = df_5[["Division", "CPI Division", "Weight_Price_Division_today"]]
        df_5.rename(columns = {'Weight_Price_Division_today':'Weight_Price_Division'}, inplace = True)
        df_5["Date"] = today_f
        ...
        # Daily change (%) of the CPI per Division 
        if today_p.weekday() == 0:
            # Execute on Monday
            previous_day_obj =  today_p - timedelta(days = 3)
        else:
