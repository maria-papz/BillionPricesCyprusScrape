...
# Merge the weights dataframe with the 'Subclass Average' column from products
df = weights.merge(products[['product_subclass', 'subclass_average']].drop_duplicates(), 
...
df['weighted.mean.price.total'] = df['weighted.mean.price'].sum()
...
df['CPI_general']=df['weighted_CPI_division'].sum()
